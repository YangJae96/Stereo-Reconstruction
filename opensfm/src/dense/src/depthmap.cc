...
DepthmapEstimator::DepthmapEstimator() // 생성자로 초기화 
    : patch_size_(7),
      min_depth_(0),
      max_depth_(0),
...
void DepthmapEstimator::AddView(const double *pK, const double *pR,
                                ...
                                int height) {
  Ks_.emplace_back(pK); // 스택처럼 push_back 
  Rs_.emplace_back(pR);
  ts_.emplace_back(pt);
...
void DepthmapEstimator::SetDepthRange(double min_depth, double max_depth,
  ...
  min_depth_ = min_depth; //24
  max_depth_ = max_depth; //96
  num_depth_planes_ = num_depth_planes; // 100
}
...
void DepthmapEstimator::ComputePatchMatchSample(
  ...
  for (int i = 0; i < patchmatch_iterations_; ++i) { // 3
...
void DepthmapCleaner::AddView(const double *pK, const double *pR,
                              ...
                              int height) {
  Ks_.emplace_back(pK);
  Rs_.emplace_back(pR); // 스택처럼 쌓기 push_back
  ts_.emplace_back(pt);
  ...
  *clean_depth = cv::Mat(depths_[0].rows, depths_[0].cols, CV_32F, 0.0f);
  ...
  for (int i = 0; i < depths_[0].rows; ++i) {
    for (int j = 0; j < depths_[0].cols; ++j) {
      ...
      cv::Vec3f point = Backproject(j, i, depth, Ks_[0], Rs_[0], ts_[0]); // 수식 (7) 
      ...
      for (int other = 1; other < depths_.size(); ++other) {
        ...
        float depth_of_point = reprojection(2);//depth_of_point == lamda*(X,Nk)
      ...
      if (consistent_views >= min_consistent_views_) //Depth-Map Refinement
      { 
